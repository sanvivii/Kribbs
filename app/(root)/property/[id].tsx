...
const ADMIN_PHONE = "919899550758"; // replace with your WhatsApp number
...
export default function PropertyDetailScreen() {
  ...
  const handleDelete = () => {
    confirmAction("Delete Property", "Are you sure?", async () => {
  ...
  const handleMarkSold = () => {
    confirmAction("Mark as Sold", "Are you sure?", async () => {
      await authSupabase
        .from("properties")
        .update({ is_sold: true })
        .eq("id", id);
  ...
  return (
    <View className="flex-1 bg-white">
        ...
        <View>
          ...
          {property.images.length > 1 && (
            <View className="absolute bottom-3 left-0 right-0 flex-row justify-center gap-1">
              {property.images.map((_: any, i: Key | null | undefined) => (
                <View
                  key={i}
      ...
      {/* Image Viewer */}
      <ImageViewing
        images={property.images.map((uri: any) => ({ uri }))}
